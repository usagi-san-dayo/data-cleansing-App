...
output_cleansingForm <- function(file, cleansingForm, fileEncoding) {
  ...
  cleanser$writeTablesOnExcel(formNumeric, formFactor, formDate, dataName2)
  cleansedData <- cleanser$dataCleanser(dataName2, fileEncoding = fileEncoding)
  save(cleansedData, file = paste0(dataName2, ".rda"))
  zip(zipfile = file, files = c(paste0("dataCleansingForm_", dataName2, "_.xlsx"), paste0(dataName2, ".rda")))
}
    ...
    tabPanel("Import", fluid = TRUE,
             sidebarLayout(
               sidebarPanel(
                 ...
                 shinyDirButton("dir", "Location", "Select the working directory", FALSE),
    ...
    tabPanel("Numeric", fluid = TRUE,
             sidebarLayout(
               sidebarPanel(
                 ...
                 checkboxGroupInput("numeric_NA", "Missing-values replace with the mean", choices = getMissValues(cleanser$cleansingForm, cleanser$cleansingForm$numeric[[1]]$colname), selected = isRepByMean(cleanser$cleansingForm, colnames(cleanser$dataset)[1]), width = "200px"),
...
server <- function(input, output, session) {
  ...
  roots <- c(home = "~", wd = ".")
  shinyDirChoose(input, "dir", roots = roots, filetypes = c("", "txt"))
  ...
  observeEvent(
    input$mkForm,{
      if (!is.null(input$file)) {
        cleanser <<- new("dataCleansing")
        cleanser$initialize()
        cleanser$mkCleansingForm(gsub("\\.csv", "", input$file$name), gsub(".csv", "", input$file$datapath), input$numFac, input$numDate, input$encode)
      }
    }
  )
  observeEvent(
    input$colName,{
      if (!is.null(cleanser$cleansingForm$numeric)) {
  ...
  output$optionsDate <- renderRHandsontable({
    if (!is.null(cleanser$cleansingForm$Date)) {
  ...
  output$downloadSave <- downloadHandler(
    ...
    content = function(file) {
      output_cleansingForm(file, cleanser$cleansingForm, input$encode)
    },
